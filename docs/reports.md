...
コンストラクタに渡した日時は、すべて UTC に直して保持します。タイムゾーンのない日時は、
実行環境のローカル時刻とみなします。受信時刻 `timestamp` と同じ規則を、発表時刻や発生時刻、
津波の到達時刻、DCX の発生日時にも当てはめています。そのため、直接生成したレポートの日時も
デコードしたレポートと同じく、常にタイムゾーン付きの UTC です。
