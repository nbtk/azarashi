...
行う操作ではなく、フィールド間の整合性は呼び出し側が用意します。
...
また、この辞書は版をまたぐ永続化形式ではありません。
南海トラフ情報は直接生成しても共有の組み立て処理に参加するため、独立したスナップショットにはなりません。
...
未知のコンストラクタ引数を付加情報として保存できるかはクラスによって異なり、共通の保証ではありません。
...
一方、`azarashi.definitions` のモジュール経路・内部の定義名、および `azarashi.decoders` の段階構成・schema・context・内部コンストラクタは互換性の対象に含めません。
