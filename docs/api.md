...
- `unique`: 重複したメッセージを無視するかどうかです。
  ...
  コールバックが正常に戻った後に、そのメッセージを通知済みとして記憶します。コールバックが失敗すると例外をそのまま呼び出し元へ伝え、後から届く同じメッセージを再び通知できます。通知中に別スレッドが読んだ重複は抑制しますが、失敗後にそのコピーを配信し直す仕組みはありません。コールバックを指定しない場合は、レポートを返す前に記憶します。
...
弱参照できないストリームは、状態を管理するためにオブジェクト自体を保持します。解放できるように、後の状態検索で `closed=True` を確認したときに記憶を破棄します。そのため、同じオブジェクトを開き直しても記憶の保持は保証しません。閉じている間に状態検索が行われなければ残ることもありますが、そのタイミングに依存しないでください。
