...
| `test` | 訓練・試験の報なら `true`。どの種類でも、このキーで見分けられます |
...
`test` は、DCR では通報区分のコードが 7（訓練/試験）のとき、DCX では A1 のコードが 0（Test）のときに `true` です。
...
日本のライブラリのコード0は、DCX 仕様書が定める「指示なし」です。`labels.en` は `No instruction` です。
国際ライブラリのコード0は、CAMF の IC-A-01（予約）で、指示なしではありません。
...
火山の日時には `activity_time_ambiguity` を必須で併記します。電文の日時の曖昧さ（Du）のコードオブジェクトで、
scheme は `qzss.dcr.ambiguity_of_activity_time` です。英語のラベルは DCR 仕様書の文言です。日本語の表はないので、`ja` はありません。
...
同じ電文は、複数の衛星から、違うプリアンブルで繰り返し届きます。そのとき、`nmea`・`received_at`・`satellite` は
レコードごとに違いますが、`type` と `data` は同じです。`type` と `data` が同じレコードは、同じ電文とみなしてください。
CLI の `--unique` と `decode_stream()` の `unique` は、同じ判断を電文のビット列で行います。

## Corrections, Cancellations and All Clears

訂正・取消・解除の報は、それが指す元の報を運びません。仕様にその項目がないからです。
報の種類・地域・時刻などの内容で、元の報と対応づけてください。

| 報 | キー | コード |
|---|---|---|
| DCR | `data.information_type` | 0 発表、1 訂正、2 取消 |
| DCX | `data.message_type` | 1 Alert、2 Update、3 All Clear、0 Test |
...
既存のキーの意味・形・単位を変えるとき、既存のキーに新しい値（`status` や `kind` の種類など）を足すとき、
キーを消すときは、別の版にします。コード表に新しいコードが加わるのは、このどれにも当たりません。
コードは `recognized` で見分けられるからです。表示文言の修正も当たりません。

`schema_version` は整数です。足すだけの変更では動かず、読む側が対応しなければならない変更でだけ上がるので、
`1.2` のような副番号は使いません。azarashi 自身のバージョンとも独立です。
