...
これは現行の仕様です。ライブ受信と古い記録の再生を、独立した組み立てとして同時に扱うことは保証しません。

`text_information` は各レポートが受信したページのバイト列です。一方、
`extract_text_information()` と `str(report)` が返す本文、および `completed` は共有状態に依存します。
後続ページの受信で、以前取得したレポートから完成した本文を読めるようになります。
新しい発表に切り替わると、以前の発表のレポートは本文の代わりに受信中の表示を返します。
`completed` もそのレポート固有の完了状態を保存した値ではありません。
表示結果をその時点の記録として残す場合は、文字列を取得して保存してください。
独立した組み立て器や、それを選ぶインスタンス API は提供しません。
