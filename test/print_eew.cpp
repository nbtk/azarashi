...
int main() {
    ...
    {
        const int notification_cap = sizeof(msg.mt43.eew.notification) / sizeof(msg.mt43.eew.notification[0]);
        int notification_count = msg.mt43.eew.notification_count;
        if (notification_count > notification_cap) {
            fprintf(stderr, "  WARNING: eew_notification_count (%u) exceeds capacity (%d), clamping\n", msg.mt43.eew.notification_count, notification_cap);
            notification_count = notification_cap;
        }
        for (int i = 0; i < notification_count; i++) {
            printf("    notification[%d]: %u\n", i, msg.mt43.eew.notification[i]);
        }
    }
    printf("  eew_quake_time: day=%u hour=%u minute=%u unix=%u\n",
           msg.mt43.eew.quake_time.day, msg.mt43.eew.quake_time.hour,
           msg.mt43.eew.quake_time.minute, msg.mt43.eew.quake_time.unix_time);
    printf("  eew_depth: %u\n", msg.mt43.eew.depth);
    ...
    {
        const int region_cap = sizeof(msg.mt43.eew.regions) / sizeof(msg.mt43.eew.regions[0]);
        int region_count = msg.mt43.eew.region_count;
        if (region_count > region_cap) {
            fprintf(stderr, "  WARNING: eew_region_count (%u) exceeds capacity (%d), clamping\n", msg.mt43.eew.region_count, region_cap);
            region_count = region_cap;
        }
        for (int i = 0; i < region_count; i++) {
            printf("    region[%d]: %u\n", i, msg.mt43.eew.regions[i]);
        }
    }
