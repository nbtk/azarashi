...
レポートの中身が JSON で表せないと、変換は素の `ValueError` や `TypeError` を送出します。タイムゾーンのない
日時、NaN や無限大、並ぶべきリストの長さ違いなどです。`AzarashiException` の外なので、「azarashi の例外は
すべて `AzarashiException` の下」という約束がここで破れています。

azarashi がデコードしたレポートでは起きないことを、8,000件の変異で確かめています。起きるとすれば、利用者が
レポートを書き換えたときです。ただ、変換の側からは、利用者の書き換えと azarashi のデコーダの不具合を見分け
られません。azarashi が呼び出しの誤りだと言い切れないので、`AzarashiFixTheCall` には入れていません。

JSON にできないものがありうること自体が別の問題だ、という見方もあります。置き場所より先に、それを議論します。
