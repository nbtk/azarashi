...
| # | 項目 | 状態 |
|---|---|---|
| 1 | D3・D4 を `quantity` の形にするか | 判断待ち |
| 2 | A11 の公開フィールド名が日本を名指し | 2か国目のライブラリが現れたら |
| 3 | ライブラリ版 #2 以降 | 仕様が公開したら |
| 4 | EX1 と JIS X 0402 の一致確認 | 確認できるまで scheme は据え置き |
| 5 | 設定ミスが `AzarashiReadOn` に入る | 非互換を扱うときに再分類 |
| 6 | 弱参照できない読み取り元の明示的解放 | 具体的な要求が出たら |
| 7 | Galileo EWS 対応 | 実際に必要になったら |

## 1. D3・D4 の形

B4 の災害別詳細のうち、D3（主楕円中心から震央への方位）と D4（その距離）だけは数値を持ち、
JSON では `{"code": {...}, "value": ..., "unit": ...}` という独自の形で出しています。
意味は `quantity` の `scalar` と同じなのに、別の型になっています。

`quantity` に寄せれば型が1つ減り、利用者は数量をすべて同じ手順で読めます。代わりに、常に
`scalar` である2項目に `kind: "scalar"` が必ず付き、単位も `semi_major_axis`（主楕円の半長軸を
単位とする相対距離）という他にない値を `quantity` の単位の列挙に加えることになります。

JSON 出力が変わるので、決めるならリリース前です。
