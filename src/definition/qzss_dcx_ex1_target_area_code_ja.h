#pragma once
// AUTO-GENERATED from azarashi 0.15.1 — do not edit
// Source module : qzss_dcx_camf_ex1_target_area_code
// Variable      : qzss_dcx_ex1_target_area_code_ja
// Entries       : 1980
// Strategy      : binary_search

#include <cstdint>

namespace azaraC {
namespace def {

struct QZSS_DCX_EX1_TARGET_AREA_CODE_JA_Entry { uint16_t id; const char* label; };
static constexpr QZSS_DCX_EX1_TARGET_AREA_CODE_JA_Entry QZSS_DCX_EX1_TARGET_AREA_CODE_JA_TABLE[] = {
    {1000u, "北海道"},

    {1100u, "北海道札幌市"},

    {1101u, "札幌市中央区"},

    {1102u, "札幌市北区"},

    {1103u, "札幌市東区"},

    {1104u, "札幌市白石区"},

    {1105u, "札幌市豊平区"},

    {1106u, "札幌市南区"},

    {1107u, "札幌市西区"},

    {1108u, "札幌市厚別区"},

    {1109u, "札幌市手稲区"},

    {1110u, "札幌市清田区"},

    {1202u, "北海道函館市"},

    {1203u, "北海道小樽市"},

    {1204u, "北海道旭川市"},

    {1205u, "北海道室蘭市"},

    {1206u, "北海道釧路市"},

    {1207u, "北海道帯広市"},

    {1208u, "北海道北見市"},

    {1209u, "北海道夕張市"},

    {1210u, "北海道岩見沢市"},

    {1211u, "北海道網走市"},

    {1212u, "北海道留萌市"},

    {1213u, "北海道苫小牧市"},

    {1214u, "北海道稚内市"},

    {1215u, "北海道美唄市"},

    {1216u, "北海道芦別市"},

    {1217u, "北海道江別市"},

    {1218u, "北海道赤平市"},

    {1219u, "北海道紋別市"},

    {1220u, "北海道士別市"},

    {1221u, "北海道名寄市"},

    {1222u, "北海道三笠市"},

    {1223u, "北海道根室市"},

    {1224u, "北海道千歳市"},

    {1225u, "北海道滝川市"},

    {1226u, "北海道砂川市"},

    {1227u, "北海道歌志内市"},

    {1228u, "北海道深川市"},

    {1229u, "北海道富良野市"},

    {1230u, "北海道登別市"},

    {1231u, "北海道恵庭市"},

    {1233u, "北海道伊達市"},

    {1234u, "北海道北広島市"},

    {1235u, "北海道石狩市"},

    {1236u, "北海道北斗市"},

    {1303u, "北海道当別町"},

    {1304u, "北海道新篠津村"},

    {1331u, "北海道松前町"},

    {1332u, "北海道福島町"},

    {1333u, "北海道知内町"},

    {1334u, "北海道木古内町"},

    {1337u, "北海道七飯町"},

    {1343u, "北海道鹿部町"},

    {1345u, "北海道森町"},

    {1346u, "北海道八雲町"},

    {1347u, "北海道長万部町"},

    {1361u, "北海道江差町"},

    {1362u, "北海道上ノ国町"},

    {1363u, "北海道厚沢部町"},

    {1364u, "北海道乙部町"},

    {1367u, "北海道奥尻町"},

    {1370u, "北海道今金町"},

    {1371u, "北海道せたな町"},

    {1391u, "北海道島牧村"},

    {1392u, "北海道寿都町"},

    {1393u, "北海道黒松内町"},

    {1394u, "北海道蘭越町"},

    {1395u, "北海道ニセコ町"},

    {1396u, "北海道真狩村"},

    {1397u, "北海道留寿都村"},

    {1398u, "北海道喜茂別町"},

    {1399u, "北海道京極町"},

    {1400u, "北海道倶知安町"},

    {1401u, "北海道共和町"},

    {1402u, "北海道岩内町"},

    {1403u, "北海道泊村"},

    {1404u, "北海道神恵内村"},

    {1405u, "北海道積丹町"},

    {1406u, "北海道古平町"},

    {1407u, "北海道仁木町"},

    {1408u, "北海道余市町"},

    {1409u, "北海道赤井川村"},

    {1423u, "北海道南幌町"},

    {1424u, "北海道奈井江町"},

    {1425u, "北海道上砂川町"},

    {1427u, "北海道由仁町"},

    {1428u, "北海道長沼町"},

    {1429u, "北海道栗山町"},

    {1430u, "北海道月形町"},

    {1431u, "北海道浦臼町"},

    {1432u, "北海道新十津川町"},

    {1433u, "北海道妹背牛町"},

    {1434u, "北海道秩父別町"},

    {1436u, "北海道雨竜町"},

    {1437u, "北海道北竜町"},

    {1438u, "北海道沼田町"},

    {1452u, "北海道鷹栖町"},

    {1453u, "北海道東神楽町"},

    {1454u, "北海道当麻町"},

    {1455u, "北海道比布町"},

    {1456u, "北海道愛別町"},

    {1457u, "北海道上川町"},

    {1458u, "北海道東川町"},

    {1459u, "北海道美瑛町"},

    {1460u, "北海道上富良野町"},

    {1461u, "北海道中富良野町"},

    {1462u, "北海道南富良野町"},

    {1463u, "北海道占冠村"},

    {1464u, "北海道和寒町"},

    {1465u, "北海道剣淵町"},

    {1468u, "北海道下川町"},

    {1469u, "北海道美深町"},

    {1470u, "北海道音威子府村"},

    {1471u, "北海道中川町"},

    {1472u, "北海道幌加内町"},

    {1481u, "北海道増毛町"},

    {1482u, "北海道小平町"},

    {1483u, "北海道苫前町"},

    {1484u, "北海道羽幌町"},

    {1485u, "北海道初山別村"},

    {1486u, "北海道遠別町"},

    {1487u, "北海道天塩町"},

    {1511u, "北海道猿払村"},

    {1512u, "北海道浜頓別町"},

    {1513u, "北海道中頓別町"},

    {1514u, "北海道枝幸町"},

    {1516u, "北海道豊富町"},

    {1517u, "北海道礼文町"},

    {1518u, "北海道利尻町"},

    {1519u, "北海道利尻富士町"},

    {1520u, "北海道幌延町"},

    {1543u, "北海道美幌町"},

    {1544u, "北海道津別町"},

    {1545u, "北海道斜里町"},

    {1546u, "北海道清里町"},

    {1547u, "北海道小清水町"},

    {1549u, "北海道訓子府町"},

    {1550u, "北海道置戸町"},

    {1552u, "北海道佐呂間町"},

    {1555u, "北海道遠軽町"},

    {1559u, "北海道湧別町"},

    {1560u, "北海道滝上町"},

    {1561u, "北海道興部町"},

    {1562u, "北海道西興部村"},

    {1563u, "北海道雄武町"},

    {1564u, "北海道大空町"},

    {1571u, "北海道豊浦町"},

    {1575u, "北海道壮瞥町"},

    {1578u, "北海道白老町"},

    {1581u, "北海道厚真町"},

    {1584u, "北海道洞爺湖町"},

    {1585u, "北海道安平町"},

    {1586u, "北海道むかわ町"},

    {1601u, "北海道日高町"},

    {1602u, "北海道平取町"},

    {1604u, "北海道新冠町"},

    {1607u, "北海道浦河町"},

    {1608u, "北海道様似町"},

    {1609u, "北海道えりも町"},

    {1610u, "北海道新ひだか町"},

    {1631u, "北海道音更町"},

    {1632u, "北海道士幌町"},

    {1633u, "北海道上士幌町"},

    {1634u, "北海道鹿追町"},

    {1635u, "北海道新得町"},

    {1636u, "北海道清水町"},

    {1637u, "北海道芽室町"},

    {1638u, "北海道中札内村"},

    {1639u, "北海道更別村"},

    {1641u, "北海道大樹町"},

    {1642u, "北海道広尾町"},

    {1643u, "北海道幕別町"},

    {1644u, "北海道池田町"},

    {1645u, "北海道豊頃町"},

    {1646u, "北海道本別町"},

    {1647u, "北海道足寄町"},

    {1648u, "北海道陸別町"},

    {1649u, "北海道浦幌町"},

    {1661u, "北海道釧路町"},

    {1662u, "北海道厚岸町"},

    {1663u, "北海道浜中町"},

    {1664u, "北海道標茶町"},

    {1665u, "北海道弟子屈町"},

    {1667u, "北海道鶴居村"},

    {1668u, "北海道白糠町"},

    {1691u, "北海道別海町"},

    {1692u, "北海道中標津町"},

    {1693u, "北海道標津町"},

    {1694u, "北海道羅臼町"},

    {1695u, "北海道色丹村"},

    {1696u, "北海道泊村"},

    {1697u, "北海道留夜別村"},

    {1698u, "北海道留別村"},

    {1699u, "北海道紗那村"},

    {1700u, "北海道蘂取村"},

    {2000u, "青森県"},

    {2201u, "青森県青森市"},

    {2202u, "青森県弘前市"},

    {2203u, "青森県八戸市"},

    {2204u, "青森県黒石市"},

    {2205u, "青森県五所川原市"},

    {2206u, "青森県十和田市"},

    {2207u, "青森県三沢市"},

    {2208u, "青森県むつ市"},

    {2209u, "青森県つがる市"},

    {2210u, "青森県平川市"},

    {2301u, "青森県平内町"},

    {2303u, "青森県今別町"},

    {2304u, "青森県蓬田村"},

    {2307u, "青森県外ヶ浜町"},

    {2321u, "青森県鰺ヶ沢町"},

    {2323u, "青森県深浦町"},

    {2343u, "青森県西目屋村"},

    {2361u, "青森県藤崎町"},

    {2362u, "青森県大鰐町"},

    {2367u, "青森県田舎館村"},

    {2381u, "青森県板柳町"},

    {2384u, "青森県鶴田町"},

    {2387u, "青森県中泊町"},

    {2401u, "青森県野辺地町"},

    {2402u, "青森県七戸町"},

    {2405u, "青森県六戸町"},

    {2406u, "青森県横浜町"},

    {2408u, "青森県東北町"},

    {2411u, "青森県六ヶ所村"},

    {2412u, "青森県おいらせ町"},

    {2423u, "青森県大間町"},

    {2424u, "青森県東通村"},

    {2425u, "青森県風間浦村"},

    {2426u, "青森県佐井村"},

    {2441u, "青森県三戸町"},

    {2442u, "青森県五戸町"},

    {2443u, "青森県田子町"},

    {2445u, "青森県南部町"},

    {2446u, "青森県階上町"},

    {2450u, "青森県新郷村"},

    {3000u, "岩手県"},

    {3201u, "岩手県盛岡市"},

    {3202u, "岩手県宮古市"},

    {3203u, "岩手県大船渡市"},

    {3205u, "岩手県花巻市"},

    {3206u, "岩手県北上市"},

    {3207u, "岩手県久慈市"},

    {3208u, "岩手県遠野市"},

    {3209u, "岩手県一関市"},

    {3210u, "岩手県陸前高田市"},

    {3211u, "岩手県釜石市"},

    {3213u, "岩手県二戸市"},

    {3214u, "岩手県八幡平市"},

    {3215u, "岩手県奥州市"},

    {3216u, "岩手県滝沢市"},

    {3301u, "岩手県雫石町"},

    {3302u, "岩手県葛巻町"},

    {3303u, "岩手県岩手町"},

    {3321u, "岩手県紫波町"},

    {3322u, "岩手県矢巾町"},

    {3366u, "岩手県西和賀町"},

    {3381u, "岩手県金ケ崎町"},

    {3402u, "岩手県平泉町"},

    {3441u, "岩手県住田町"},

    {3461u, "岩手県大槌町"},

    {3482u, "岩手県山田町"},

    {3483u, "岩手県岩泉町"},

    {3484u, "岩手県田野畑村"},

    {3485u, "岩手県普代村"},

    {3501u, "岩手県軽米町"},

    {3503u, "岩手県野田村"},

    {3506u, "岩手県九戸村"},

    {3507u, "岩手県洋野町"},

    {3524u, "岩手県一戸町"},

    {4000u, "宮城県"},

    {4100u, "宮城県仙台市"},

    {4101u, "仙台市青葉区"},

    {4102u, "仙台市宮城野区"},

    {4103u, "仙台市若林区"},

    {4104u, "仙台市太白区"},

    {4105u, "仙台市泉区"},

    {4202u, "宮城県石巻市"},

    {4203u, "宮城県塩竈市"},

    {4205u, "宮城県気仙沼市"},

    {4206u, "宮城県白石市"},

    {4207u, "宮城県名取市"},

    {4208u, "宮城県角田市"},

    {4209u, "宮城県多賀城市"},

    {4211u, "宮城県岩沼市"},

    {4212u, "宮城県登米市"},

    {4213u, "宮城県栗原市"},

    {4214u, "宮城県東松島市"},

    {4215u, "宮城県大崎市"},

    {4216u, "宮城県富谷市"},

    {4301u, "宮城県蔵王町"},

    {4302u, "宮城県七ヶ宿町"},

    {4321u, "宮城県大河原町"},

    {4322u, "宮城県村田町"},

    {4323u, "宮城県柴田町"},

    {4324u, "宮城県川崎町"},

    {4341u, "宮城県丸森町"},

    {4361u, "宮城県亘理町"},

    {4362u, "宮城県山元町"},

    {4401u, "宮城県松島町"},

    {4404u, "宮城県七ヶ浜町"},

    {4406u, "宮城県利府町"},

    {4421u, "宮城県大和町"},

    {4422u, "宮城県大郷町"},

    {4424u, "宮城県大衡村"},

    {4444u, "宮城県色麻町"},

    {4445u, "宮城県加美町"},

    {4501u, "宮城県涌谷町"},

    {4505u, "宮城県美里町"},

    {4581u, "宮城県女川町"},

    {4606u, "宮城県南三陸町"},

    {5000u, "秋田県"},

    {5201u, "秋田県秋田市"},

    {5202u, "秋田県能代市"},

    {5203u, "秋田県横手市"},

    {5204u, "秋田県大館市"},

    {5206u, "秋田県男鹿市"},

    {5207u, "秋田県湯沢市"},

    {5209u, "秋田県鹿角市"},

    {5210u, "秋田県由利本荘市"},

    {5211u, "秋田県潟上市"},

    {5212u, "秋田県大仙市"},

    {5213u, "秋田県北秋田市"},

    {5214u, "秋田県にかほ市"},

    {5215u, "秋田県仙北市"},

    {5303u, "秋田県小坂町"},

    {5327u, "秋田県上小阿仁村"},

    {5346u, "秋田県藤里町"},

    {5348u, "秋田県三種町"},

    {5349u, "秋田県八峰町"},

    {5361u, "秋田県五城目町"},

    {5363u, "秋田県八郎潟町"},

    {5366u, "秋田県井川町"},

    {5368u, "秋田県大潟村"},

    {5434u, "秋田県美郷町"},

    {5463u, "秋田県羽後町"},

    {5464u, "秋田県東成瀬村"},

    {6000u, "山形県"},

    {6201u, "山形県山形市"},

    {6202u, "山形県米沢市"},

    {6203u, "山形県鶴岡市"},

    {6204u, "山形県酒田市"},

    {6205u, "山形県新庄市"},

    {6206u, "山形県寒河江市"},

    {6207u, "山形県上山市"},

    {6208u, "山形県村山市"},

    {6209u, "山形県長井市"},

    {6210u, "山形県天童市"},

    {6211u, "山形県東根市"},

    {6212u, "山形県尾花沢市"},

    {6213u, "山形県南陽市"},

    {6301u, "山形県山辺町"},

    {6302u, "山形県中山町"},

    {6321u, "山形県河北町"},

    {6322u, "山形県西川町"},

    {6323u, "山形県朝日町"},

    {6324u, "山形県大江町"},

    {6341u, "山形県大石田町"},

    {6361u, "山形県金山町"},

    {6362u, "山形県最上町"},

    {6363u, "山形県舟形町"},

    {6364u, "山形県真室川町"},

    {6365u, "山形県大蔵村"},

    {6366u, "山形県鮭川村"},

    {6367u, "山形県戸沢村"},

    {6381u, "山形県高畠町"},

    {6382u, "山形県川西町"},

    {6401u, "山形県小国町"},

    {6402u, "山形県白鷹町"},

    {6403u, "山形県飯豊町"},

    {6426u, "山形県三川町"},

    {6428u, "山形県庄内町"},

    {6461u, "山形県遊佐町"},

    {7000u, "福島県"},

    {7201u, "福島県福島市"},

    {7202u, "福島県会津若松市"},

    {7203u, "福島県郡山市"},

    {7204u, "福島県いわき市"},

    {7205u, "福島県白河市"},

    {7207u, "福島県須賀川市"},

    {7208u, "福島県喜多方市"},

    {7209u, "福島県相馬市"},

    {7210u, "福島県二本松市"},

    {7211u, "福島県田村市"},

    {7212u, "福島県南相馬市"},

    {7213u, "福島県伊達市"},

    {7214u, "福島県本宮市"},

    {7301u, "福島県桑折町"},

    {7303u, "福島県国見町"},

    {7308u, "福島県川俣町"},

    {7322u, "福島県大玉村"},

    {7342u, "福島県鏡石町"},

    {7344u, "福島県天栄村"},

    {7362u, "福島県下郷町"},

    {7364u, "福島県檜枝岐村"},

    {7367u, "福島県只見町"},

    {7368u, "福島県南会津町"},

    {7402u, "福島県北塩原村"},

    {7405u, "福島県西会津町"},

    {7407u, "福島県磐梯町"},

    {7408u, "福島県猪苗代町"},

    {7421u, "福島県会津坂下町"},

    {7422u, "福島県湯川村"},

    {7423u, "福島県柳津町"},

    {7444u, "福島県三島町"},

    {7445u, "福島県金山町"},

    {7446u, "福島県昭和村"},

    {7447u, "福島県会津美里町"},

    {7461u, "福島県西郷村"},

    {7464u, "福島県泉崎村"},

    {7465u, "福島県中島村"},

    {7466u, "福島県矢吹町"},

    {7481u, "福島県棚倉町"},

    {7482u, "福島県矢祭町"},

    {7483u, "福島県塙町"},

    {7484u, "福島県鮫川村"},

    {7501u, "福島県石川町"},

    {7502u, "福島県玉川村"},

    {7503u, "福島県平田村"},

    {7504u, "福島県浅川町"},

    {7505u, "福島県古殿町"},

    {7521u, "福島県三春町"},

    {7522u, "福島県小野町"},

    {7541u, "福島県広野町"},

    {7542u, "福島県楢葉町"},

    {7543u, "福島県富岡町"},

    {7544u, "福島県川内村"},

    {7545u, "福島県大熊町"},

    {7546u, "福島県双葉町"},

    {7547u, "福島県浪江町"},

    {7548u, "福島県葛尾村"},

    {7561u, "福島県新地町"},

    {7564u, "福島県飯舘村"},

    {8000u, "茨城県"},

    {8201u, "茨城県水戸市"},

    {8202u, "茨城県日立市"},

    {8203u, "茨城県土浦市"},

    {8204u, "茨城県古河市"},

    {8205u, "茨城県石岡市"},

    {8207u, "茨城県結城市"},

    {8208u, "茨城県龍ケ崎市"},

    {8210u, "茨城県下妻市"},

    {8211u, "茨城県常総市"},

    {8212u, "茨城県常陸太田市"},

    {8214u, "茨城県高萩市"},

    {8215u, "茨城県北茨城市"},

    {8216u, "茨城県笠間市"},

    {8217u, "茨城県取手市"},

    {8219u, "茨城県牛久市"},

    {8220u, "茨城県つくば市"},

    {8221u, "茨城県ひたちなか市"},

    {8222u, "茨城県鹿嶋市"},

    {8223u, "茨城県潮来市"},

    {8224u, "茨城県守谷市"},

    {8225u, "茨城県常陸大宮市"},

    {8226u, "茨城県那珂市"},

    {8227u, "茨城県筑西市"},

    {8228u, "茨城県坂東市"},

    {8229u, "茨城県稲敷市"},

    {8230u, "茨城県かすみがうら市"},

    {8231u, "茨城県桜川市"},

    {8232u, "茨城県神栖市"},

    {8233u, "茨城県行方市"},

    {8234u, "茨城県鉾田市"},

    {8235u, "茨城県つくばみらい市"},

    {8236u, "茨城県小美玉市"},

    {8302u, "茨城県茨城町"},

    {8309u, "茨城県大洗町"},

    {8310u, "茨城県城里町"},

    {8341u, "茨城県東海村"},

    {8364u, "茨城県大子町"},

    {8442u, "茨城県美浦村"},

    {8443u, "茨城県阿見町"},

    {8447u, "茨城県河内町"},

    {8521u, "茨城県八千代町"},

    {8542u, "茨城県五霞町"},

    {8546u, "茨城県境町"},

    {8564u, "茨城県利根町"},

    {9000u, "栃木県"},

    {9201u, "栃木県宇都宮市"},

    {9202u, "栃木県足利市"},

    {9203u, "栃木県栃木市"},

    {9204u, "栃木県佐野市"},

    {9205u, "栃木県鹿沼市"},

    {9206u, "栃木県日光市"},

    {9208u, "栃木県小山市"},

    {9209u, "栃木県真岡市"},

    {9210u, "栃木県大田原市"},

    {9211u, "栃木県矢板市"},

    {9213u, "栃木県那須塩原市"},

    {9214u, "栃木県さくら市"},

    {9215u, "栃木県那須烏山市"},

    {9216u, "栃木県下野市"},

    {9301u, "栃木県上三川町"},

    {9342u, "栃木県益子町"},

    {9343u, "栃木県茂木町"},

    {9344u, "栃木県市貝町"},

    {9345u, "栃木県芳賀町"},

    {9361u, "栃木県壬生町"},

    {9364u, "栃木県野木町"},

    {9384u, "栃木県塩谷町"},

    {9386u, "栃木県高根沢町"},

    {9407u, "栃木県那須町"},

    {9411u, "栃木県那珂川町"},

    {10000u, "群馬県"},

    {10201u, "群馬県前橋市"},

    {10202u, "群馬県高崎市"},

    {10203u, "群馬県桐生市"},

    {10204u, "群馬県伊勢崎市"},

    {10205u, "群馬県太田市"},

    {10206u, "群馬県沼田市"},

    {10207u, "群馬県館林市"},

    {10208u, "群馬県渋川市"},

    {10209u, "群馬県藤岡市"},

    {10210u, "群馬県富岡市"},

    {10211u, "群馬県安中市"},

    {10212u, "群馬県みどり市"},

    {10344u, "群馬県榛東村"},

    {10345u, "群馬県吉岡町"},

    {10366u, "群馬県上野村"},

    {10367u, "群馬県神流町"},

    {10382u, "群馬県下仁田町"},

    {10383u, "群馬県南牧村"},

    {10384u, "群馬県甘楽町"},

    {10421u, "群馬県中之条町"},

    {10424u, "群馬県長野原町"},

    {10425u, "群馬県嬬恋村"},

    {10426u, "群馬県草津町"},

    {10428u, "群馬県高山村"},

    {10429u, "群馬県東吾妻町"},

    {10443u, "群馬県片品村"},

    {10444u, "群馬県川場村"},

    {10448u, "群馬県昭和村"},

    {10449u, "群馬県みなかみ町"},

    {10464u, "群馬県玉村町"},

    {10521u, "群馬県板倉町"},

    {10522u, "群馬県明和町"},

    {10523u, "群馬県千代田町"},

    {10524u, "群馬県大泉町"},

    {10525u, "群馬県邑楽町"},

    {11000u, "埼玉県"},

    {11100u, "埼玉県さいたま市"},

    {11101u, "さいたま市西区"},

    {11102u, "さいたま市北区"},

    {11103u, "さいたま市大宮区"},

    {11104u, "さいたま市見沼区"},

    {11105u, "さいたま市中央区"},

    {11106u, "さいたま市桜区"},

    {11107u, "さいたま市浦和区"},

    {11108u, "さいたま市南区"},

    {11109u, "さいたま市緑区"},

    {11110u, "さいたま市岩槻区"},

    {11201u, "埼玉県川越市"},

    {11202u, "埼玉県熊谷市"},

    {11203u, "埼玉県川口市"},

    {11206u, "埼玉県行田市"},

    {11207u, "埼玉県秩父市"},

    {11208u, "埼玉県所沢市"},

    {11209u, "埼玉県飯能市"},

    {11210u, "埼玉県加須市"},

    {11211u, "埼玉県本庄市"},

    {11212u, "埼玉県東松山市"},

    {11214u, "埼玉県春日部市"},

    {11215u, "埼玉県狭山市"},

    {11216u, "埼玉県羽生市"},

    {11217u, "埼玉県鴻巣市"},

    {11218u, "埼玉県深谷市"},

    {11219u, "埼玉県上尾市"},

    {11221u, "埼玉県草加市"},

    {11222u, "埼玉県越谷市"},

    {11223u, "埼玉県蕨市"},

    {11224u, "埼玉県戸田市"},

    {11225u, "埼玉県入間市"},

    {11227u, "埼玉県朝霞市"},

    {11228u, "埼玉県志木市"},

    {11229u, "埼玉県和光市"},

    {11230u, "埼玉県新座市"},

    {11231u, "埼玉県桶川市"},

    {11232u, "埼玉県久喜市"},

    {11233u, "埼玉県北本市"},

    {11234u, "埼玉県八潮市"},

    {11235u, "埼玉県富士見市"},

    {11237u, "埼玉県三郷市"},

    {11238u, "埼玉県蓮田市"},

    {11239u, "埼玉県坂戸市"},

    {11240u, "埼玉県幸手市"},

    {11241u, "埼玉県鶴ヶ島市"},

    {11242u, "埼玉県日高市"},

    {11243u, "埼玉県吉川市"},

    {11245u, "埼玉県ふじみ野市"},

    {11246u, "埼玉県白岡市"},

    {11301u, "埼玉県伊奈町"},

    {11324u, "埼玉県三芳町"},

    {11326u, "埼玉県毛呂山町"},

    {11327u, "埼玉県越生町"},

    {11341u, "埼玉県滑川町"},

    {11342u, "埼玉県嵐山町"},

    {11343u, "埼玉県小川町"},

    {11346u, "埼玉県川島町"},

    {11347u, "埼玉県吉見町"},

    {11348u, "埼玉県鳩山町"},

    {11349u, "埼玉県ときがわ町"},

    {11361u, "埼玉県横瀬町"},

    {11362u, "埼玉県皆野町"},

    {11363u, "埼玉県長瀞町"},

    {11365u, "埼玉県小鹿野町"},

    {11369u, "埼玉県東秩父村"},

    {11381u, "埼玉県美里町"},

    {11383u, "埼玉県神川町"},

    {11385u, "埼玉県上里町"},

    {11408u, "埼玉県寄居町"},

    {11442u, "埼玉県宮代町"},

    {11464u, "埼玉県杉戸町"},

    {11465u, "埼玉県松伏町"},

    {12000u, "千葉県"},

    {12100u, "千葉県千葉市"},

    {12101u, "千葉市中央区"},

    {12102u, "千葉市花見川区"},

    {12103u, "千葉市稲毛区"},

    {12104u, "千葉市若葉区"},

    {12105u, "千葉市緑区"},

    {12106u, "千葉市美浜区"},

    {12202u, "千葉県銚子市"},

    {12203u, "千葉県市川市"},

    {12204u, "千葉県船橋市"},

    {12205u, "千葉県館山市"},

    {12206u, "千葉県木更津市"},

    {12207u, "千葉県松戸市"},

    {12208u, "千葉県野田市"},

    {12210u, "千葉県茂原市"},

    {12211u, "千葉県成田市"},

    {12212u, "千葉県佐倉市"},

    {12213u, "千葉県東金市"},

    {12215u, "千葉県旭市"},

    {12216u, "千葉県習志野市"},

    {12217u, "千葉県柏市"},

    {12218u, "千葉県勝浦市"},

    {12219u, "千葉県市原市"},

    {12220u, "千葉県流山市"},

    {12221u, "千葉県八千代市"},

    {12222u, "千葉県我孫子市"},

    {12223u, "千葉県鴨川市"},

    {12224u, "千葉県鎌ケ谷市"},

    {12225u, "千葉県君津市"},

    {12226u, "千葉県富津市"},

    {12227u, "千葉県浦安市"},

    {12228u, "千葉県四街道市"},

    {12229u, "千葉県袖ケ浦市"},

    {12230u, "千葉県八街市"},

    {12231u, "千葉県印西市"},

    {12232u, "千葉県白井市"},

    {12233u, "千葉県富里市"},

    {12234u, "千葉県南房総市"},

    {12235u, "千葉県匝瑳市"},

    {12236u, "千葉県香取市"},

    {12237u, "千葉県山武市"},

    {12238u, "千葉県いすみ市"},

    {12239u, "千葉県大網白里市"},

    {12322u, "千葉県酒々井町"},

    {12329u, "千葉県栄町"},

    {12342u, "千葉県神崎町"},

    {12347u, "千葉県多古町"},

    {12349u, "千葉県東庄町"},

    {12403u, "千葉県九十九里町"},

    {12409u, "千葉県芝山町"},

    {12410u, "千葉県横芝光町"},

    {12421u, "千葉県一宮町"},

    {12422u, "千葉県睦沢町"},

    {12423u, "千葉県長生村"},

    {12424u, "千葉県白子町"},

    {12426u, "千葉県長柄町"},

    {12427u, "千葉県長南町"},

    {12441u, "千葉県大多喜町"},

    {12443u, "千葉県御宿町"},

    {12463u, "千葉県鋸南町"},

    {13000u, "東京都"},

    {13101u, "東京都千代田区"},

    {13102u, "東京都中央区"},

    {13103u, "東京都港区"},

    {13104u, "東京都新宿区"},

    {13105u, "東京都文京区"},

    {13106u, "東京都台東区"},

    {13107u, "東京都墨田区"},

    {13108u, "東京都江東区"},

    {13109u, "東京都品川区"},

    {13110u, "東京都目黒区"},

    {13111u, "東京都大田区"},

    {13112u, "東京都世田谷区"},

    {13113u, "東京都渋谷区"},

    {13114u, "東京都中野区"},

    {13115u, "東京都杉並区"},

    {13116u, "東京都豊島区"},

    {13117u, "東京都北区"},

    {13118u, "東京都荒川区"},

    {13119u, "東京都板橋区"},

    {13120u, "東京都練馬区"},

    {13121u, "東京都足立区"},

    {13122u, "東京都葛飾区"},

    {13123u, "東京都江戸川区"},

    {13201u, "東京都八王子市"},

    {13202u, "東京都立川市"},

    {13203u, "東京都武蔵野市"},

    {13204u, "東京都三鷹市"},

    {13205u, "東京都青梅市"},

    {13206u, "東京都府中市"},

    {13207u, "東京都昭島市"},

    {13208u, "東京都調布市"},

    {13209u, "東京都町田市"},

    {13210u, "東京都小金井市"},

    {13211u, "東京都小平市"},

    {13212u, "東京都日野市"},

    {13213u, "東京都東村山市"},

    {13214u, "東京都国分寺市"},

    {13215u, "東京都国立市"},

    {13218u, "東京都福生市"},

    {13219u, "東京都狛江市"},

    {13220u, "東京都東大和市"},

    {13221u, "東京都清瀬市"},

    {13222u, "東京都東久留米市"},

    {13223u, "東京都武蔵村山市"},

    {13224u, "東京都多摩市"},

    {13225u, "東京都稲城市"},

    {13227u, "東京都羽村市"},

    {13228u, "東京都あきる野市"},

    {13229u, "東京都西東京市"},

    {13303u, "東京都瑞穂町"},

    {13305u, "東京都日の出町"},

    {13307u, "東京都檜原村"},

    {13308u, "東京都奥多摩町"},

    {13361u, "東京都大島町"},

    {13362u, "東京都利島村"},

    {13363u, "東京都新島村"},

    {13364u, "東京都神津島村"},

    {13381u, "東京都三宅村"},

    {13382u, "東京都御蔵島村"},

    {13401u, "東京都八丈町"},

    {13402u, "東京都青ヶ島村"},

    {13421u, "東京都小笠原村"},

    {14000u, "神奈川県"},

    {14100u, "神奈川県横浜市"},

    {14101u, "横浜市鶴見区"},

    {14102u, "横浜市神奈川区"},

    {14103u, "横浜市西区"},

    {14104u, "横浜市中区"},

    {14105u, "横浜市南区"},

    {14106u, "横浜市保土ケ谷区"},

    {14107u, "横浜市磯子区"},

    {14108u, "横浜市金沢区"},

    {14109u, "横浜市港北区"},

    {14110u, "横浜市戸塚区"},

    {14111u, "横浜市港南区"},

    {14112u, "横浜市旭区"},

    {14113u, "横浜市緑区"},

    {14114u, "横浜市瀬谷区"},

    {14115u, "横浜市栄区"},

    {14116u, "横浜市泉区"},

    {14117u, "横浜市青葉区"},

    {14118u, "横浜市都筑区"},

    {14130u, "神奈川県川崎市"},

    {14131u, "川崎市川崎区"},

    {14132u, "川崎市幸区"},

    {14133u, "川崎市中原区"},

    {14134u, "川崎市高津区"},

    {14135u, "川崎市多摩区"},

    {14136u, "川崎市宮前区"},

    {14137u, "川崎市麻生区"},

    {14150u, "神奈川県相模原市"},

    {14151u, "相模原市緑区"},

    {14152u, "相模原市中央区"},

    {14153u, "相模原市南区"},

    {14201u, "神奈川県横須賀市"},

    {14203u, "神奈川県平塚市"},

    {14204u, "神奈川県鎌倉市"},

    {14205u, "神奈川県藤沢市"},

    {14206u, "神奈川県小田原市"},

    {14207u, "神奈川県茅ヶ崎市"},

    {14208u, "神奈川県逗子市"},

    {14210u, "神奈川県三浦市"},

    {14211u, "神奈川県秦野市"},

    {14212u, "神奈川県厚木市"},

    {14213u, "神奈川県大和市"},

    {14214u, "神奈川県伊勢原市"},

    {14215u, "神奈川県海老名市"},

    {14216u, "神奈川県座間市"},

    {14217u, "神奈川県南足柄市"},

    {14218u, "神奈川県綾瀬市"},

    {14301u, "神奈川県葉山町"},

    {14321u, "神奈川県寒川町"},

    {14341u, "神奈川県大磯町"},

    {14342u, "神奈川県二宮町"},

    {14361u, "神奈川県中井町"},

    {14362u, "神奈川県大井町"},

    {14363u, "神奈川県松田町"},

    {14364u, "神奈川県山北町"},

    {14366u, "神奈川県開成町"},

    {14382u, "神奈川県箱根町"},

    {14383u, "神奈川県真鶴町"},

    {14384u, "神奈川県湯河原町"},

    {14401u, "神奈川県愛川町"},

    {14402u, "神奈川県清川村"},

    {15000u, "新潟県"},

    {15100u, "新潟県新潟市"},

    {15101u, "新潟市北区"},

    {15102u, "新潟市東区"},

    {15103u, "新潟市中央区"},

    {15104u, "新潟市江南区"},

    {15105u, "新潟市秋葉区"},

    {15106u, "新潟市南区"},

    {15107u, "新潟市西区"},

    {15108u, "新潟市西蒲区"},

    {15202u, "新潟県長岡市"},

    {15204u, "新潟県三条市"},

    {15205u, "新潟県柏崎市"},

    {15206u, "新潟県新発田市"},

    {15208u, "新潟県小千谷市"},

    {15209u, "新潟県加茂市"},

    {15210u, "新潟県十日町市"},

    {15211u, "新潟県見附市"},

    {15212u, "新潟県村上市"},

    {15213u, "新潟県燕市"},

    {15216u, "新潟県糸魚川市"},

    {15217u, "新潟県妙高市"},

    {15218u, "新潟県五泉市"},

    {15222u, "新潟県上越市"},

    {15223u, "新潟県阿賀野市"},

    {15224u, "新潟県佐渡市"},

    {15225u, "新潟県魚沼市"},

    {15226u, "新潟県南魚沼市"},

    {15227u, "新潟県胎内市"},

    {15307u, "新潟県聖籠町"},

    {15342u, "新潟県弥彦村"},

    {15361u, "新潟県田上町"},

    {15385u, "新潟県阿賀町"},

    {15405u, "新潟県出雲崎町"},

    {15461u, "新潟県湯沢町"},

    {15482u, "新潟県津南町"},

    {15504u, "新潟県刈羽村"},

    {15581u, "新潟県関川村"},

    {15586u, "新潟県粟島浦村"},

    {16000u, "富山県"},

    {16201u, "富山県富山市"},

    {16202u, "富山県高岡市"},

    {16204u, "富山県魚津市"},

    {16205u, "富山県氷見市"},

    {16206u, "富山県滑川市"},

    {16207u, "富山県黒部市"},

    {16208u, "富山県砺波市"},

    {16209u, "富山県小矢部市"},

    {16210u, "富山県南砺市"},

    {16211u, "富山県射水市"},

    {16321u, "富山県舟橋村"},

    {16322u, "富山県上市町"},

    {16323u, "富山県立山町"},

    {16342u, "富山県入善町"},

    {16343u, "富山県朝日町"},

    {17000u, "石川県"},

    {17201u, "石川県金沢市"},

    {17202u, "石川県七尾市"},

    {17203u, "石川県小松市"},

    {17204u, "石川県輪島市"},

    {17205u, "石川県珠洲市"},

    {17206u, "石川県加賀市"},

    {17207u, "石川県羽咋市"},

    {17209u, "石川県かほく市"},

    {17210u, "石川県白山市"},

    {17211u, "石川県能美市"},

    {17212u, "石川県野々市市"},

    {17324u, "石川県川北町"},

    {17361u, "石川県津幡町"},

    {17365u, "石川県内灘町"},

    {17384u, "石川県志賀町"},

    {17386u, "石川県宝達志水町"},

    {17407u, "石川県中能登町"},

    {17461u, "石川県穴水町"},

    {17463u, "石川県能登町"},

    {18000u, "福井県"},

    {18201u, "福井県福井市"},

    {18202u, "福井県敦賀市"},

    {18204u, "福井県小浜市"},

    {18205u, "福井県大野市"},

    {18206u, "福井県勝山市"},

    {18207u, "福井県鯖江市"},

    {18208u, "福井県あわら市"},

    {18209u, "福井県越前市"},

    {18210u, "福井県坂井市"},

    {18322u, "福井県永平寺町"},

    {18382u, "福井県池田町"},

    {18404u, "福井県南越前町"},

    {18423u, "福井県越前町"},

    {18442u, "福井県美浜町"},

    {18481u, "福井県高浜町"},

    {18483u, "福井県おおい町"},

    {18501u, "福井県若狭町"},

    {19000u, "山梨県"},

    {19201u, "山梨県甲府市"},

    {19202u, "山梨県富士吉田市"},

    {19204u, "山梨県都留市"},

    {19205u, "山梨県山梨市"},

    {19206u, "山梨県大月市"},

    {19207u, "山梨県韮崎市"},

    {19208u, "山梨県南アルプス市"},

    {19209u, "山梨県北杜市"},

    {19210u, "山梨県甲斐市"},

    {19211u, "山梨県笛吹市"},

    {19212u, "山梨県上野原市"},

    {19213u, "山梨県甲州市"},

    {19214u, "山梨県中央市"},

    {19346u, "山梨県市川三郷町"},

    {19364u, "山梨県早川町"},

    {19365u, "山梨県身延町"},

    {19366u, "山梨県南部町"},

    {19368u, "山梨県富士川町"},

    {19384u, "山梨県昭和町"},

    {19422u, "山梨県道志村"},

    {19423u, "山梨県西桂町"},

    {19424u, "山梨県忍野村"},

    {19425u, "山梨県山中湖村"},

    {19429u, "山梨県鳴沢村"},

    {19430u, "山梨県富士河口湖町"},

    {19442u, "山梨県小菅村"},

    {19443u, "山梨県丹波山村"},

    {20000u, "長野県"},

    {20201u, "長野県長野市"},

    {20202u, "長野県松本市"},

    {20203u, "長野県上田市"},

    {20204u, "長野県岡谷市"},

    {20205u, "長野県飯田市"},

    {20206u, "長野県諏訪市"},

    {20207u, "長野県須坂市"},

    {20208u, "長野県小諸市"},

    {20209u, "長野県伊那市"},

    {20210u, "長野県駒ヶ根市"},

    {20211u, "長野県中野市"},

    {20212u, "長野県大町市"},

    {20213u, "長野県飯山市"},

    {20214u, "長野県茅野市"},

    {20215u, "長野県塩尻市"},

    {20217u, "長野県佐久市"},

    {20218u, "長野県千曲市"},

    {20219u, "長野県東御市"},

    {20220u, "長野県安曇野市"},

    {20303u, "長野県小海町"},

    {20304u, "長野県川上村"},

    {20305u, "長野県南牧村"},

    {20306u, "長野県南相木村"},

    {20307u, "長野県北相木村"},

    {20309u, "長野県佐久穂町"},

    {20321u, "長野県軽井沢町"},

    {20323u, "長野県御代田町"},

    {20324u, "長野県立科町"},

    {20349u, "長野県青木村"},

    {20350u, "長野県長和町"},

    {20361u, "長野県下諏訪町"},

    {20362u, "長野県富士見町"},

    {20363u, "長野県原村"},

    {20382u, "長野県辰野町"},

    {20383u, "長野県箕輪町"},

    {20384u, "長野県飯島町"},

    {20385u, "長野県南箕輪村"},

    {20386u, "長野県中川村"},

    {20388u, "長野県宮田村"},

    {20402u, "長野県松川町"},

    {20403u, "長野県高森町"},

    {20404u, "長野県阿南町"},

    {20407u, "長野県阿智村"},

    {20409u, "長野県平谷村"},

    {20410u, "長野県根羽村"},

    {20411u, "長野県下條村"},

    {20412u, "長野県売木村"},

    {20413u, "長野県天龍村"},

    {20414u, "長野県泰阜村"},

    {20415u, "長野県喬木村"},

    {20416u, "長野県豊丘村"},

    {20417u, "長野県大鹿村"},

    {20422u, "長野県上松町"},

    {20423u, "長野県南木曽町"},

    {20425u, "長野県木祖村"},

    {20429u, "長野県王滝村"},

    {20430u, "長野県大桑村"},

    {20432u, "長野県木曽町"},

    {20446u, "長野県麻績村"},

    {20448u, "長野県生坂村"},

    {20450u, "長野県山形村"},

    {20451u, "長野県朝日村"},

    {20452u, "長野県筑北村"},

    {20481u, "長野県池田町"},

    {20482u, "長野県松川村"},

    {20485u, "長野県白馬村"},

    {20486u, "長野県小谷村"},

    {20521u, "長野県坂城町"},

    {20541u, "長野県小布施町"},

    {20543u, "長野県高山村"},

    {20561u, "長野県山ノ内町"},

    {20562u, "長野県木島平村"},

    {20563u, "長野県野沢温泉村"},

    {20583u, "長野県信濃町"},

    {20588u, "長野県小川村"},

    {20590u, "長野県飯綱町"},

    {20602u, "長野県栄村"},

    {21000u, "岐阜県"},

    {21201u, "岐阜県岐阜市"},

    {21202u, "岐阜県大垣市"},

    {21203u, "岐阜県高山市"},

    {21204u, "岐阜県多治見市"},

    {21205u, "岐阜県関市"},

    {21206u, "岐阜県中津川市"},

    {21207u, "岐阜県美濃市"},

    {21208u, "岐阜県瑞浪市"},

    {21209u, "岐阜県羽島市"},

    {21210u, "岐阜県恵那市"},

    {21211u, "岐阜県美濃加茂市"},

    {21212u, "岐阜県土岐市"},

    {21213u, "岐阜県各務原市"},

    {21214u, "岐阜県可児市"},

    {21215u, "岐阜県山県市"},

    {21216u, "岐阜県瑞穂市"},

    {21217u, "岐阜県飛騨市"},

    {21218u, "岐阜県本巣市"},

    {21219u, "岐阜県郡上市"},

    {21220u, "岐阜県下呂市"},

    {21221u, "岐阜県海津市"},

    {21302u, "岐阜県岐南町"},

    {21303u, "岐阜県笠松町"},

    {21341u, "岐阜県養老町"},

    {21361u, "岐阜県垂井町"},

    {21362u, "岐阜県関ケ原町"},

    {21381u, "岐阜県神戸町"},

    {21382u, "岐阜県輪之内町"},

    {21383u, "岐阜県安八町"},

    {21401u, "岐阜県揖斐川町"},

    {21403u, "岐阜県大野町"},

    {21404u, "岐阜県池田町"},

    {21421u, "岐阜県北方町"},

    {21501u, "岐阜県坂祝町"},

    {21502u, "岐阜県富加町"},

    {21503u, "岐阜県川辺町"},

    {21504u, "岐阜県七宗町"},

    {21505u, "岐阜県八百津町"},

    {21506u, "岐阜県白川町"},

    {21507u, "岐阜県東白川村"},

    {21521u, "岐阜県御嵩町"},

    {21604u, "岐阜県白川村"},

    {22000u, "静岡県"},

    {22100u, "静岡県静岡市"},

    {22101u, "静岡市葵区"},

    {22102u, "静岡市駿河区"},

    {22103u, "静岡市清水区"},

    {22130u, "静岡県浜松市"},

    {22131u, "浜松市中区"},

    {22132u, "浜松市東区"},

    {22133u, "浜松市西区"},

    {22134u, "浜松市南区"},

    {22135u, "浜松市北区"},

    {22136u, "浜松市浜北区"},

    {22137u, "浜松市天竜区"},

    {22138u, "浜松市中央区"},

    {22139u, "浜松市浜名区"},

    {22140u, "浜松市天竜区"},

    {22203u, "静岡県沼津市"},

    {22205u, "静岡県熱海市"},

    {22206u, "静岡県三島市"},

    {22207u, "静岡県富士宮市"},

    {22208u, "静岡県伊東市"},

    {22209u, "静岡県島田市"},

    {22210u, "静岡県富士市"},

    {22211u, "静岡県磐田市"},

    {22212u, "静岡県焼津市"},

    {22213u, "静岡県掛川市"},

    {22214u, "静岡県藤枝市"},

    {22215u, "静岡県御殿場市"},

    {22216u, "静岡県袋井市"},

    {22219u, "静岡県下田市"},

    {22220u, "静岡県裾野市"},

    {22221u, "静岡県湖西市"},

    {22222u, "静岡県伊豆市"},

    {22223u, "静岡県御前崎市"},

    {22224u, "静岡県菊川市"},

    {22225u, "静岡県伊豆の国市"},

    {22226u, "静岡県牧之原市"},

    {22301u, "静岡県東伊豆町"},

    {22302u, "静岡県河津町"},

    {22304u, "静岡県南伊豆町"},

    {22305u, "静岡県松崎町"},

    {22306u, "静岡県西伊豆町"},

    {22325u, "静岡県函南町"},

    {22341u, "静岡県清水町"},

    {22342u, "静岡県長泉町"},

    {22344u, "静岡県小山町"},

    {22424u, "静岡県吉田町"},

    {22429u, "静岡県川根本町"},

    {22461u, "静岡県森町"},

    {23000u, "愛知県"},

    {23100u, "愛知県名古屋市"},

    {23101u, "名古屋市千種区"},

    {23102u, "名古屋市東区"},

    {23103u, "名古屋市北区"},

    {23104u, "名古屋市西区"},

    {23105u, "名古屋市中村区"},

    {23106u, "名古屋市中区"},

    {23107u, "名古屋市昭和区"},

    {23108u, "名古屋市瑞穂区"},

    {23109u, "名古屋市熱田区"},

    {23110u, "名古屋市中川区"},

    {23111u, "名古屋市港区"},

    {23112u, "名古屋市南区"},

    {23113u, "名古屋市守山区"},

    {23114u, "名古屋市緑区"},

    {23115u, "名古屋市名東区"},

    {23116u, "名古屋市天白区"},

    {23201u, "愛知県豊橋市"},

    {23202u, "愛知県岡崎市"},

    {23203u, "愛知県一宮市"},

    {23204u, "愛知県瀬戸市"},

    {23205u, "愛知県半田市"},

    {23206u, "愛知県春日井市"},

    {23207u, "愛知県豊川市"},

    {23208u, "愛知県津島市"},

    {23209u, "愛知県碧南市"},

    {23210u, "愛知県刈谷市"},

    {23211u, "愛知県豊田市"},

    {23212u, "愛知県安城市"},

    {23213u, "愛知県西尾市"},

    {23214u, "愛知県蒲郡市"},

    {23215u, "愛知県犬山市"},

    {23216u, "愛知県常滑市"},

    {23217u, "愛知県江南市"},

    {23219u, "愛知県小牧市"},

    {23220u, "愛知県稲沢市"},

    {23221u, "愛知県新城市"},

    {23222u, "愛知県東海市"},

    {23223u, "愛知県大府市"},

    {23224u, "愛知県知多市"},

    {23225u, "愛知県知立市"},

    {23226u, "愛知県尾張旭市"},

    {23227u, "愛知県高浜市"},

    {23228u, "愛知県岩倉市"},

    {23229u, "愛知県豊明市"},

    {23230u, "愛知県日進市"},

    {23231u, "愛知県田原市"},

    {23232u, "愛知県愛西市"},

    {23233u, "愛知県清須市"},

    {23234u, "愛知県北名古屋市"},

    {23235u, "愛知県弥富市"},

    {23236u, "愛知県みよし市"},

    {23237u, "愛知県あま市"},

    {23238u, "愛知県長久手市"},

    {23302u, "愛知県東郷町"},

    {23342u, "愛知県豊山町"},

    {23361u, "愛知県大口町"},

    {23362u, "愛知県扶桑町"},

    {23424u, "愛知県大治町"},

    {23425u, "愛知県蟹江町"},

    {23427u, "愛知県飛島村"},

    {23441u, "愛知県阿久比町"},

    {23442u, "愛知県東浦町"},

    {23445u, "愛知県南知多町"},

    {23446u, "愛知県美浜町"},

    {23447u, "愛知県武豊町"},

    {23501u, "愛知県幸田町"},

    {23561u, "愛知県設楽町"},

    {23562u, "愛知県東栄町"},

    {23563u, "愛知県豊根村"},

    {24000u, "三重県"},

    {24201u, "三重県津市"},

    {24202u, "三重県四日市市"},

    {24203u, "三重県伊勢市"},

    {24204u, "三重県松阪市"},

    {24205u, "三重県桑名市"},

    {24207u, "三重県鈴鹿市"},

    {24208u, "三重県名張市"},

    {24209u, "三重県尾鷲市"},

    {24210u, "三重県亀山市"},

    {24211u, "三重県鳥羽市"},

    {24212u, "三重県熊野市"},

    {24214u, "三重県いなべ市"},

    {24215u, "三重県志摩市"},

    {24216u, "三重県伊賀市"},

    {24303u, "三重県木曽岬町"},

    {24324u, "三重県東員町"},

    {24341u, "三重県菰野町"},

    {24343u, "三重県朝日町"},

    {24344u, "三重県川越町"},

    {24441u, "三重県多気町"},

    {24442u, "三重県明和町"},

    {24443u, "三重県大台町"},

    {24461u, "三重県玉城町"},

    {24470u, "三重県度会町"},

    {24471u, "三重県大紀町"},

    {24472u, "三重県南伊勢町"},

    {24543u, "三重県紀北町"},

    {24561u, "三重県御浜町"},

    {24562u, "三重県紀宝町"},

    {25000u, "滋賀県"},

    {25201u, "滋賀県大津市"},

    {25202u, "滋賀県彦根市"},

    {25203u, "滋賀県長浜市"},

    {25204u, "滋賀県近江八幡市"},

    {25206u, "滋賀県草津市"},

    {25207u, "滋賀県守山市"},

    {25208u, "滋賀県栗東市"},

    {25209u, "滋賀県甲賀市"},

    {25210u, "滋賀県野洲市"},

    {25211u, "滋賀県湖南市"},

    {25212u, "滋賀県高島市"},

    {25213u, "滋賀県東近江市"},

    {25214u, "滋賀県米原市"},

    {25383u, "滋賀県日野町"},

    {25384u, "滋賀県竜王町"},

    {25425u, "滋賀県愛荘町"},

    {25441u, "滋賀県豊郷町"},

    {25442u, "滋賀県甲良町"},

    {25443u, "滋賀県多賀町"},

    {26000u, "京都府"},

    {26100u, "京都府京都市"},

    {26101u, "京都市北区"},

    {26102u, "京都市上京区"},

    {26103u, "京都市左京区"},

    {26104u, "京都市中京区"},

    {26105u, "京都市東山区"},

    {26106u, "京都市下京区"},

    {26107u, "京都市南区"},

    {26108u, "京都市右京区"},

    {26109u, "京都市伏見区"},

    {26110u, "京都市山科区"},

    {26111u, "京都市西京区"},

    {26201u, "京都府福知山市"},

    {26202u, "京都府舞鶴市"},

    {26203u, "京都府綾部市"},

    {26204u, "京都府宇治市"},

    {26205u, "京都府宮津市"},

    {26206u, "京都府亀岡市"},

    {26207u, "京都府城陽市"},

    {26208u, "京都府向日市"},

    {26209u, "京都府長岡京市"},

    {26210u, "京都府八幡市"},

    {26211u, "京都府京田辺市"},

    {26212u, "京都府京丹後市"},

    {26213u, "京都府南丹市"},

    {26214u, "京都府木津川市"},

    {26303u, "京都府大山崎町"},

    {26322u, "京都府久御山町"},

    {26343u, "京都府井手町"},

    {26344u, "京都府宇治田原町"},

    {26364u, "京都府笠置町"},

    {26365u, "京都府和束町"},

    {26366u, "京都府精華町"},

    {26367u, "京都府南山城村"},

    {26407u, "京都府京丹波町"},

    {26463u, "京都府伊根町"},

    {26465u, "京都府与謝野町"},

    {27000u, "大阪府"},

    {27100u, "大阪府大阪市"},

    {27101u, "大阪市北区"},

    {27102u, "大阪市都島区"},

    {27103u, "大阪市福島区"},

    {27104u, "大阪市此花区"},

    {27105u, "大阪市東区"},

    {27106u, "大阪市西区"},

    {27107u, "大阪市港区"},

    {27108u, "大阪市大正区"},

    {27109u, "大阪市天王寺区"},

    {27110u, "大阪市南区"},

    {27111u, "大阪市浪速区"},

    {27112u, "大阪市大淀区"},

    {27113u, "大阪市西淀川区"},

    {27114u, "大阪市東淀川区"},

    {27115u, "大阪市東成区"},

    {27116u, "大阪市生野区"},

    {27117u, "大阪市旭区"},

    {27118u, "大阪市城東区"},

    {27119u, "大阪市阿倍野区"},

    {27120u, "大阪市住吉区"},

    {27121u, "大阪市東住吉区"},

    {27122u, "大阪市西成区"},

    {27123u, "大阪市淀川区"},

    {27124u, "大阪市鶴見区"},

    {27125u, "大阪市住之江区"},

    {27126u, "大阪市平野区"},

    {27127u, "大阪市北区"},

    {27128u, "大阪市中央区"},

    {27140u, "大阪府堺市"},

    {27141u, "堺市堺区"},

    {27142u, "堺市中区"},

    {27143u, "堺市東区"},

    {27144u, "堺市西区"},

    {27145u, "堺市南区"},

    {27146u, "堺市北区"},

    {27147u, "堺市美原区"},

    {27202u, "大阪府岸和田市"},

    {27203u, "大阪府豊中市"},

    {27204u, "大阪府池田市"},

    {27205u, "大阪府吹田市"},

    {27206u, "大阪府泉大津市"},

    {27207u, "大阪府高槻市"},

    {27208u, "大阪府貝塚市"},

    {27209u, "大阪府守口市"},

    {27210u, "大阪府枚方市"},

    {27211u, "大阪府茨木市"},

    {27212u, "大阪府八尾市"},

    {27213u, "大阪府泉佐野市"},

    {27214u, "大阪府富田林市"},

    {27215u, "大阪府寝屋川市"},

    {27216u, "大阪府河内長野市"},

    {27217u, "大阪府松原市"},

    {27218u, "大阪府大東市"},

    {27219u, "大阪府和泉市"},

    {27220u, "大阪府箕面市"},

    {27221u, "大阪府柏原市"},

    {27222u, "大阪府羽曳野市"},

    {27223u, "大阪府門真市"},

    {27224u, "大阪府摂津市"},

    {27225u, "大阪府高石市"},

    {27226u, "大阪府藤井寺市"},

    {27227u, "大阪府東大阪市"},

    {27228u, "大阪府泉南市"},

    {27229u, "大阪府四條畷市"},

    {27230u, "大阪府交野市"},

    {27231u, "大阪府大阪狭山市"},

    {27232u, "大阪府阪南市"},

    {27301u, "大阪府島本町"},

    {27321u, "大阪府豊能町"},

    {27322u, "大阪府能勢町"},

    {27341u, "大阪府忠岡町"},

    {27361u, "大阪府熊取町"},

    {27362u, "大阪府田尻町"},

    {27366u, "大阪府岬町"},

    {27381u, "大阪府太子町"},

    {27382u, "大阪府河南町"},

    {27383u, "大阪府千早赤阪村"},

    {28000u, "兵庫県"},

    {28100u, "兵庫県神戸市"},

    {28101u, "神戸市東灘区"},

    {28102u, "神戸市灘区"},

    {28103u, "神戸市葺合区"},

    {28104u, "神戸市生田区"},

    {28105u, "神戸市兵庫区"},

    {28106u, "神戸市長田区"},

    {28107u, "神戸市須磨区"},

    {28108u, "神戸市垂水区"},

    {28109u, "神戸市北区"},

    {28110u, "神戸市中央区"},

    {28111u, "神戸市西区"},

    {28201u, "兵庫県姫路市"},

    {28202u, "兵庫県尼崎市"},

    {28203u, "兵庫県明石市"},

    {28204u, "兵庫県西宮市"},

    {28205u, "兵庫県洲本市"},

    {28206u, "兵庫県芦屋市"},

    {28207u, "兵庫県伊丹市"},

    {28208u, "兵庫県相生市"},

    {28209u, "兵庫県豊岡市"},

    {28210u, "兵庫県加古川市"},

    {28212u, "兵庫県赤穂市"},

    {28213u, "兵庫県西脇市"},

    {28214u, "兵庫県宝塚市"},

    {28215u, "兵庫県三木市"},

    {28216u, "兵庫県高砂市"},

    {28217u, "兵庫県川西市"},

    {28218u, "兵庫県小野市"},

    {28219u, "兵庫県三田市"},

    {28220u, "兵庫県加西市"},

    {28221u, "兵庫県丹波篠山市"},

    {28222u, "兵庫県養父市"},

    {28223u, "兵庫県丹波市"},

    {28224u, "兵庫県南あわじ市"},

    {28225u, "兵庫県朝来市"},

    {28226u, "兵庫県淡路市"},

    {28227u, "兵庫県宍粟市"},

    {28228u, "兵庫県加東市"},

    {28229u, "兵庫県たつの市"},

    {28301u, "兵庫県猪名川町"},

    {28365u, "兵庫県多可町"},

    {28381u, "兵庫県稲美町"},

    {28382u, "兵庫県播磨町"},

    {28442u, "兵庫県市川町"},

    {28443u, "兵庫県福崎町"},

    {28446u, "兵庫県神河町"},

    {28464u, "兵庫県太子町"},

    {28481u, "兵庫県上郡町"},

    {28501u, "兵庫県佐用町"},

    {28585u, "兵庫県香美町"},

    {28586u, "兵庫県新温泉町"},

    {29000u, "奈良県"},

    {29201u, "奈良県奈良市"},

    {29202u, "奈良県大和高田市"},

    {29203u, "奈良県大和郡山市"},

    {29204u, "奈良県天理市"},

    {29205u, "奈良県橿原市"},

    {29206u, "奈良県桜井市"},

    {29207u, "奈良県五條市"},

    {29208u, "奈良県御所市"},

    {29209u, "奈良県生駒市"},

    {29210u, "奈良県香芝市"},

    {29211u, "奈良県葛城市"},

    {29212u, "奈良県宇陀市"},

    {29322u, "奈良県山添村"},

    {29342u, "奈良県平群町"},

    {29343u, "奈良県三郷町"},

    {29344u, "奈良県斑鳩町"},

    {29345u, "奈良県安堵町"},

    {29361u, "奈良県川西町"},

    {29362u, "奈良県三宅町"},

    {29363u, "奈良県田原本町"},

    {29385u, "奈良県曽爾村"},

    {29386u, "奈良県御杖村"},

    {29401u, "奈良県高取町"},

    {29402u, "奈良県明日香村"},

    {29424u, "奈良県上牧町"},

    {29425u, "奈良県王寺町"},

    {29426u, "奈良県広陵町"},

    {29427u, "奈良県河合町"},

    {29441u, "奈良県吉野町"},

    {29442u, "奈良県大淀町"},

    {29443u, "奈良県下市町"},

    {29444u, "奈良県黒滝村"},

    {29446u, "奈良県天川村"},

    {29447u, "奈良県野迫川村"},

    {29449u, "奈良県十津川村"},

    {29450u, "奈良県下北山村"},

    {29451u, "奈良県上北山村"},

    {29452u, "奈良県川上村"},

    {29453u, "奈良県東吉野村"},

    {30000u, "和歌山県"},

    {30201u, "和歌山県和歌山市"},

    {30202u, "和歌山県海南市"},

    {30203u, "和歌山県橋本市"},

    {30204u, "和歌山県有田市"},

    {30205u, "和歌山県御坊市"},

    {30206u, "和歌山県田辺市"},

    {30207u, "和歌山県新宮市"},

    {30208u, "和歌山県紀の川市"},

    {30209u, "和歌山県岩出市"},

    {30304u, "和歌山県紀美野町"},

    {30341u, "和歌山県かつらぎ町"},

    {30343u, "和歌山県九度山町"},

    {30344u, "和歌山県高野町"},

    {30361u, "和歌山県湯浅町"},

    {30362u, "和歌山県広川町"},

    {30366u, "和歌山県有田川町"},

    {30381u, "和歌山県美浜町"},

    {30382u, "和歌山県日高町"},

    {30383u, "和歌山県由良町"},

    {30390u, "和歌山県印南町"},

    {30391u, "和歌山県みなべ町"},

    {30392u, "和歌山県日高川町"},

    {30401u, "和歌山県白浜町"},

    {30404u, "和歌山県上富田町"},

    {30406u, "和歌山県すさみ町"},

    {30421u, "和歌山県那智勝浦町"},

    {30422u, "和歌山県太地町"},

    {30424u, "和歌山県古座川町"},

    {30427u, "和歌山県北山村"},

    {30428u, "和歌山県串本町"},

    {31000u, "鳥取県"},

    {31201u, "鳥取県鳥取市"},

    {31202u, "鳥取県米子市"},

    {31203u, "鳥取県倉吉市"},

    {31204u, "鳥取県境港市"},

    {31302u, "鳥取県岩美町"},

    {31325u, "鳥取県若桜町"},

    {31328u, "鳥取県智頭町"},

    {31329u, "鳥取県八頭町"},

    {31364u, "鳥取県三朝町"},

    {31370u, "鳥取県湯梨浜町"},

    {31371u, "鳥取県琴浦町"},

    {31372u, "鳥取県北栄町"},

    {31384u, "鳥取県日吉津村"},

    {31386u, "鳥取県大山町"},

    {31389u, "鳥取県南部町"},

    {31390u, "鳥取県伯耆町"},

    {31401u, "鳥取県日南町"},

    {31402u, "鳥取県日野町"},

    {31403u, "鳥取県江府町"},

    {32000u, "島根県"},

    {32201u, "島根県松江市"},

    {32202u, "島根県浜田市"},

    {32203u, "島根県出雲市"},

    {32204u, "島根県益田市"},

    {32205u, "島根県大田市"},

    {32206u, "島根県安来市"},

    {32207u, "島根県江津市"},

    {32209u, "島根県雲南市"},

    {32343u, "島根県奥出雲町"},

    {32386u, "島根県飯南町"},

    {32441u, "島根県川本町"},

    {32448u, "島根県美郷町"},

    {32449u, "島根県邑南町"},

    {32501u, "島根県津和野町"},

    {32505u, "島根県吉賀町"},

    {32525u, "島根県海士町"},

    {32526u, "島根県西ノ島町"},

    {32527u, "島根県知夫村"},

    {32528u, "島根県隠岐の島町"},

    {33000u, "岡山県"},

    {33100u, "岡山県岡山市"},

    {33101u, "岡山市北区"},

    {33102u, "岡山市中区"},

    {33103u, "岡山市東区"},

    {33104u, "岡山市南区"},

    {33202u, "岡山県倉敷市"},

    {33203u, "岡山県津山市"},

    {33204u, "岡山県玉野市"},

    {33205u, "岡山県笠岡市"},

    {33207u, "岡山県井原市"},

    {33208u, "岡山県総社市"},

    {33209u, "岡山県高梁市"},

    {33210u, "岡山県新見市"},

    {33211u, "岡山県備前市"},

    {33212u, "岡山県瀬戸内市"},

    {33213u, "岡山県赤磐市"},

    {33214u, "岡山県真庭市"},

    {33215u, "岡山県美作市"},

    {33216u, "岡山県浅口市"},

    {33346u, "岡山県和気町"},

    {33423u, "岡山県早島町"},

    {33445u, "岡山県里庄町"},

    {33461u, "岡山県矢掛町"},

    {33586u, "岡山県新庄村"},

    {33606u, "岡山県鏡野町"},

    {33622u, "岡山県勝央町"},

    {33623u, "岡山県奈義町"},

    {33643u, "岡山県西粟倉村"},

    {33663u, "岡山県久米南町"},

    {33666u, "岡山県美咲町"},

    {33681u, "岡山県吉備中央町"},

    {34000u, "広島県"},

    {34100u, "広島県広島市"},

    {34101u, "広島市中区"},

    {34102u, "広島市東区"},

    {34103u, "広島市南区"},

    {34104u, "広島市西区"},

    {34105u, "広島市安佐南区"},

    {34106u, "広島市安佐北区"},

    {34107u, "広島市安芸区"},

    {34108u, "広島市佐伯区"},

    {34202u, "広島県呉市"},

    {34203u, "広島県竹原市"},

    {34204u, "広島県三原市"},

    {34205u, "広島県尾道市"},

    {34207u, "広島県福山市"},

    {34208u, "広島県府中市"},

    {34209u, "広島県三次市"},

    {34210u, "広島県庄原市"},

    {34211u, "広島県大竹市"},

    {34212u, "広島県東広島市"},

    {34213u, "広島県廿日市市"},

    {34214u, "広島県安芸高田市"},

    {34215u, "広島県江田島市"},

    {34302u, "広島県府中町"},

    {34304u, "広島県海田町"},

    {34307u, "広島県熊野町"},

    {34309u, "広島県坂町"},

    {34368u, "広島県安芸太田町"},

    {34369u, "広島県北広島町"},

    {34431u, "広島県大崎上島町"},

    {34462u, "広島県世羅町"},

    {34545u, "広島県神石高原町"},

    {35000u, "山口県"},

    {35201u, "山口県下関市"},

    {35202u, "山口県宇部市"},

    {35203u, "山口県山口市"},

    {35204u, "山口県萩市"},

    {35206u, "山口県防府市"},

    {35207u, "山口県下松市"},

    {35208u, "山口県岩国市"},

    {35210u, "山口県光市"},

    {35211u, "山口県長門市"},

    {35212u, "山口県柳井市"},

    {35213u, "山口県美祢市"},

    {35215u, "山口県周南市"},

    {35216u, "山口県山陽小野田市"},

    {35305u, "山口県周防大島町"},

    {35321u, "山口県和木町"},

    {35341u, "山口県上関町"},

    {35343u, "山口県田布施町"},

    {35344u, "山口県平生町"},

    {35502u, "山口県阿武町"},

    {36000u, "徳島県"},

    {36201u, "徳島県徳島市"},

    {36202u, "徳島県鳴門市"},

    {36203u, "徳島県小松島市"},

    {36204u, "徳島県阿南市"},

    {36205u, "徳島県吉野川市"},

    {36206u, "徳島県阿波市"},

    {36207u, "徳島県美馬市"},

    {36208u, "徳島県三好市"},

    {36301u, "徳島県勝浦町"},

    {36302u, "徳島県上勝町"},

    {36321u, "徳島県佐那河内村"},

    {36341u, "徳島県石井町"},

    {36342u, "徳島県神山町"},

    {36368u, "徳島県那賀町"},

    {36383u, "徳島県牟岐町"},

    {36387u, "徳島県美波町"},

    {36388u, "徳島県海陽町"},

    {36401u, "徳島県松茂町"},

    {36402u, "徳島県北島町"},

    {36403u, "徳島県藍住町"},

    {36404u, "徳島県板野町"},

    {36405u, "徳島県上板町"},

    {36468u, "徳島県つるぎ町"},

    {36489u, "徳島県東みよし町"},

    {37000u, "香川県"},

    {37201u, "香川県高松市"},

    {37202u, "香川県丸亀市"},

    {37203u, "香川県坂出市"},

    {37204u, "香川県善通寺市"},

    {37205u, "香川県観音寺市"},

    {37206u, "香川県さぬき市"},

    {37207u, "香川県東かがわ市"},

    {37208u, "香川県三豊市"},

    {37322u, "香川県土庄町"},

    {37324u, "香川県小豆島町"},

    {37341u, "香川県三木町"},

    {37364u, "香川県直島町"},

    {37386u, "香川県宇多津町"},

    {37387u, "香川県綾川町"},

    {37403u, "香川県琴平町"},

    {37404u, "香川県多度津町"},

    {37406u, "香川県まんのう町"},

    {38000u, "愛媛県"},

    {38201u, "愛媛県松山市"},

    {38202u, "愛媛県今治市"},

    {38203u, "愛媛県宇和島市"},

    {38204u, "愛媛県八幡浜市"},

    {38205u, "愛媛県新居浜市"},

    {38206u, "愛媛県西条市"},

    {38207u, "愛媛県大洲市"},

    {38210u, "愛媛県伊予市"},

    {38213u, "愛媛県四国中央市"},

    {38214u, "愛媛県西予市"},

    {38215u, "愛媛県東温市"},

    {38356u, "愛媛県上島町"},

    {38386u, "愛媛県久万高原町"},

    {38401u, "愛媛県松前町"},

    {38402u, "愛媛県砥部町"},

    {38422u, "愛媛県内子町"},

    {38442u, "愛媛県伊方町"},

    {38484u, "愛媛県松野町"},

    {38488u, "愛媛県鬼北町"},

    {38506u, "愛媛県愛南町"},

    {39000u, "高知県"},

    {39201u, "高知県高知市"},

    {39202u, "高知県室戸市"},

    {39203u, "高知県安芸市"},

    {39204u, "高知県南国市"},

    {39205u, "高知県土佐市"},

    {39206u, "高知県須崎市"},

    {39208u, "高知県宿毛市"},

    {39209u, "高知県土佐清水市"},

    {39210u, "高知県四万十市"},

    {39211u, "高知県香南市"},

    {39212u, "高知県香美市"},

    {39301u, "高知県東洋町"},

    {39302u, "高知県奈半利町"},

    {39303u, "高知県田野町"},

    {39304u, "高知県安田町"},

    {39305u, "高知県北川村"},

    {39306u, "高知県馬路村"},

    {39307u, "高知県芸西村"},

    {39341u, "高知県本山町"},

    {39344u, "高知県大豊町"},

    {39363u, "高知県土佐町"},

    {39364u, "高知県大川村"},

    {39386u, "高知県いの町"},

    {39387u, "高知県仁淀川町"},

    {39401u, "高知県中土佐町"},

    {39402u, "高知県佐川町"},

    {39403u, "高知県越知町"},

    {39405u, "高知県梼原町"},

    {39410u, "高知県日高村"},

    {39411u, "高知県津野町"},

    {39412u, "高知県四万十町"},

    {39424u, "高知県大月町"},

    {39427u, "高知県三原村"},

    {39428u, "高知県黒潮町"},

    {40000u, "福岡県"},

    {40100u, "福岡県北九州市"},

    {40101u, "北九州市門司区"},

    {40102u, "北九州市小倉区"},

    {40103u, "北九州市若松区"},

    {40104u, "北九州市八幡区"},

    {40105u, "北九州市戸畑区"},

    {40106u, "北九州市小倉北区"},

    {40107u, "北九州市小倉南区"},

    {40108u, "北九州市八幡東区"},

    {40109u, "北九州市八幡西区"},

    {40130u, "福岡県福岡市"},

    {40131u, "福岡市東区"},

    {40132u, "福岡市博多区"},

    {40133u, "福岡市中央区"},

    {40134u, "福岡市南区"},

    {40135u, "福岡市西区"},

    {40136u, "福岡市城南区"},

    {40137u, "福岡市早良区"},

    {40202u, "福岡県大牟田市"},

    {40203u, "福岡県久留米市"},

    {40204u, "福岡県直方市"},

    {40205u, "福岡県飯塚市"},

    {40206u, "福岡県田川市"},

    {40207u, "福岡県柳川市"},

    {40210u, "福岡県八女市"},

    {40211u, "福岡県筑後市"},

    {40212u, "福岡県大川市"},

    {40213u, "福岡県行橋市"},

    {40214u, "福岡県豊前市"},

    {40215u, "福岡県中間市"},

    {40216u, "福岡県小郡市"},

    {40217u, "福岡県筑紫野市"},

    {40218u, "福岡県春日市"},

    {40219u, "福岡県大野城市"},

    {40220u, "福岡県宗像市"},

    {40221u, "福岡県太宰府市"},

    {40223u, "福岡県古賀市"},

    {40224u, "福岡県福津市"},

    {40225u, "福岡県うきは市"},

    {40226u, "福岡県宮若市"},

    {40227u, "福岡県嘉麻市"},

    {40228u, "福岡県朝倉市"},

    {40229u, "福岡県みやま市"},

    {40230u, "福岡県糸島市"},

    {40231u, "福岡県那珂川市"},

    {40341u, "福岡県宇美町"},

    {40342u, "福岡県篠栗町"},

    {40343u, "福岡県志免町"},

    {40344u, "福岡県須恵町"},

    {40345u, "福岡県新宮町"},

    {40348u, "福岡県久山町"},

    {40349u, "福岡県粕屋町"},

    {40381u, "福岡県芦屋町"},

    {40382u, "福岡県水巻町"},

    {40383u, "福岡県岡垣町"},

    {40384u, "福岡県遠賀町"},

    {40401u, "福岡県小竹町"},

    {40402u, "福岡県鞍手町"},

    {40421u, "福岡県桂川町"},

    {40447u, "福岡県筑前町"},

    {40448u, "福岡県東峰村"},

    {40503u, "福岡県大刀洗町"},

    {40522u, "福岡県大木町"},

    {40544u, "福岡県広川町"},

    {40601u, "福岡県香春町"},

    {40602u, "福岡県添田町"},

    {40604u, "福岡県糸田町"},

    {40605u, "福岡県川崎町"},

    {40608u, "福岡県大任町"},

    {40609u, "福岡県赤村"},

    {40610u, "福岡県福智町"},

    {40621u, "福岡県苅田町"},

    {40625u, "福岡県みやこ町"},

    {40642u, "福岡県吉富町"},

    {40646u, "福岡県上毛町"},

    {40647u, "福岡県築上町"},

    {41000u, "佐賀県"},

    {41201u, "佐賀県佐賀市"},

    {41202u, "佐賀県唐津市"},

    {41203u, "佐賀県鳥栖市"},

    {41204u, "佐賀県多久市"},

    {41205u, "佐賀県伊万里市"},

    {41206u, "佐賀県武雄市"},

    {41207u, "佐賀県鹿島市"},

    {41208u, "佐賀県小城市"},

    {41209u, "佐賀県嬉野市"},

    {41210u, "佐賀県神埼市"},

    {41327u, "佐賀県吉野ヶ里町"},

    {41341u, "佐賀県基山町"},

    {41345u, "佐賀県上峰町"},

    {41346u, "佐賀県みやき町"},

    {41387u, "佐賀県玄海町"},

    {41401u, "佐賀県有田町"},

    {41423u, "佐賀県大町町"},

    {41424u, "佐賀県江北町"},

    {41425u, "佐賀県白石町"},

    {41441u, "佐賀県太良町"},

    {42000u, "長崎県"},

    {42201u, "長崎県長崎市"},

    {42202u, "長崎県佐世保市"},

    {42203u, "長崎県島原市"},

    {42204u, "長崎県諫早市"},

    {42205u, "長崎県大村市"},

    {42207u, "長崎県平戸市"},

    {42208u, "長崎県松浦市"},

    {42209u, "長崎県対馬市"},

    {42210u, "長崎県壱岐市"},

    {42211u, "長崎県五島市"},

    {42212u, "長崎県西海市"},

    {42213u, "長崎県雲仙市"},

    {42214u, "長崎県南島原市"},

    {42307u, "長崎県長与町"},

    {42308u, "長崎県時津町"},

    {42321u, "長崎県東彼杵町"},

    {42322u, "長崎県川棚町"},

    {42323u, "長崎県波佐見町"},

    {42383u, "長崎県小値賀町"},

    {42391u, "長崎県佐々町"},

    {42411u, "長崎県新上五島町"},

    {43000u, "熊本県"},

    {43100u, "熊本県熊本市"},

    {43101u, "熊本市中央区"},

    {43102u, "熊本市東区"},

    {43103u, "熊本市西区"},

    {43104u, "熊本市南区"},

    {43105u, "熊本市北区"},

    {43202u, "熊本県八代市"},

    {43203u, "熊本県人吉市"},

    {43204u, "熊本県荒尾市"},

    {43205u, "熊本県水俣市"},

    {43206u, "熊本県玉名市"},

    {43208u, "熊本県山鹿市"},

    {43210u, "熊本県菊池市"},

    {43211u, "熊本県宇土市"},

    {43212u, "熊本県上天草市"},

    {43213u, "熊本県宇城市"},

    {43214u, "熊本県阿蘇市"},

    {43215u, "熊本県天草市"},

    {43216u, "熊本県合志市"},

    {43348u, "熊本県美里町"},

    {43364u, "熊本県玉東町"},

    {43367u, "熊本県南関町"},

    {43368u, "熊本県長洲町"},

    {43369u, "熊本県和水町"},

    {43403u, "熊本県大津町"},

    {43404u, "熊本県菊陽町"},

    {43423u, "熊本県南小国町"},

    {43424u, "熊本県小国町"},

    {43425u, "熊本県産山村"},

    {43428u, "熊本県高森町"},

    {43432u, "熊本県西原村"},

    {43433u, "熊本県南阿蘇村"},

    {43441u, "熊本県御船町"},

    {43442u, "熊本県嘉島町"},

    {43443u, "熊本県益城町"},

    {43444u, "熊本県甲佐町"},

    {43447u, "熊本県山都町"},

    {43468u, "熊本県氷川町"},

    {43482u, "熊本県芦北町"},

    {43484u, "熊本県津奈木町"},

    {43501u, "熊本県錦町"},

    {43505u, "熊本県多良木町"},

    {43506u, "熊本県湯前町"},

    {43507u, "熊本県水上村"},

    {43510u, "熊本県相良村"},

    {43511u, "熊本県五木村"},

    {43512u, "熊本県山江村"},

    {43513u, "熊本県球磨村"},

    {43514u, "熊本県あさぎり町"},

    {43531u, "熊本県苓北町"},

    {44000u, "大分県"},

    {44201u, "大分県大分市"},

    {44202u, "大分県別府市"},

    {44203u, "大分県中津市"},

    {44204u, "大分県日田市"},

    {44205u, "大分県佐伯市"},

    {44206u, "大分県臼杵市"},

    {44207u, "大分県津久見市"},

    {44208u, "大分県竹田市"},

    {44209u, "大分県豊後高田市"},

    {44210u, "大分県杵築市"},

    {44211u, "大分県宇佐市"},

    {44212u, "大分県豊後大野市"},

    {44213u, "大分県由布市"},

    {44214u, "大分県国東市"},

    {44322u, "大分県姫島村"},

    {44341u, "大分県日出町"},

    {44461u, "大分県九重町"},

    {44462u, "大分県玖珠町"},

    {45000u, "宮崎県"},

    {45201u, "宮崎県宮崎市"},

    {45202u, "宮崎県都城市"},

    {45203u, "宮崎県延岡市"},

    {45204u, "宮崎県日南市"},

    {45205u, "宮崎県小林市"},

    {45206u, "宮崎県日向市"},

    {45207u, "宮崎県串間市"},

    {45208u, "宮崎県西都市"},

    {45209u, "宮崎県えびの市"},

    {45341u, "宮崎県三股町"},

    {45361u, "宮崎県高原町"},

    {45382u, "宮崎県国富町"},

    {45383u, "宮崎県綾町"},

    {45401u, "宮崎県高鍋町"},

    {45402u, "宮崎県新富町"},

    {45403u, "宮崎県西米良村"},

    {45404u, "宮崎県木城町"},

    {45405u, "宮崎県川南町"},

    {45406u, "宮崎県都農町"},

    {45421u, "宮崎県門川町"},

    {45429u, "宮崎県諸塚村"},

    {45430u, "宮崎県椎葉村"},

    {45431u, "宮崎県美郷町"},

    {45441u, "宮崎県高千穂町"},

    {45442u, "宮崎県日之影町"},

    {45443u, "宮崎県五ヶ瀬町"},

    {46000u, "鹿児島県"},

    {46201u, "鹿児島県鹿児島市"},

    {46203u, "鹿児島県鹿屋市"},

    {46204u, "鹿児島県枕崎市"},

    {46206u, "鹿児島県阿久根市"},

    {46208u, "鹿児島県出水市"},

    {46210u, "鹿児島県指宿市"},

    {46213u, "鹿児島県西之表市"},

    {46214u, "鹿児島県垂水市"},

    {46215u, "鹿児島県薩摩川内市"},

    {46216u, "鹿児島県日置市"},

    {46217u, "鹿児島県曽於市"},

    {46218u, "鹿児島県霧島市"},

    {46219u, "鹿児島県いちき串木野市"},

    {46220u, "鹿児島県南さつま市"},

    {46221u, "鹿児島県志布志市"},

    {46222u, "鹿児島県奄美市"},

    {46223u, "鹿児島県南九州市"},

    {46224u, "鹿児島県伊佐市"},

    {46225u, "鹿児島県姶良市"},

    {46303u, "鹿児島県三島村"},

    {46304u, "鹿児島県十島村"},

    {46392u, "鹿児島県さつま町"},

    {46404u, "鹿児島県長島町"},

    {46452u, "鹿児島県湧水町"},

    {46468u, "鹿児島県大崎町"},

    {46482u, "鹿児島県東串良町"},

    {46490u, "鹿児島県錦江町"},

    {46491u, "鹿児島県南大隅町"},

    {46492u, "鹿児島県肝付町"},

    {46501u, "鹿児島県中種子町"},

    {46502u, "鹿児島県南種子町"},

    {46505u, "鹿児島県屋久島町"},

    {46523u, "鹿児島県大和村"},

    {46524u, "鹿児島県宇検村"},

    {46525u, "鹿児島県瀬戸内町"},

    {46527u, "鹿児島県龍郷町"},

    {46529u, "鹿児島県喜界町"},

    {46530u, "鹿児島県徳之島町"},

    {46531u, "鹿児島県天城町"},

    {46532u, "鹿児島県伊仙町"},

    {46533u, "鹿児島県和泊町"},

    {46534u, "鹿児島県知名町"},

    {46535u, "鹿児島県与論町"},

    {47000u, "沖縄県"},

    {47201u, "沖縄県那覇市"},

    {47205u, "沖縄県宜野湾市"},

    {47207u, "沖縄県石垣市"},

    {47208u, "沖縄県浦添市"},

    {47209u, "沖縄県名護市"},

    {47210u, "沖縄県糸満市"},

    {47211u, "沖縄県沖縄市"},

    {47212u, "沖縄県豊見城市"},

    {47213u, "沖縄県うるま市"},

    {47214u, "沖縄県宮古島市"},

    {47215u, "沖縄県南城市"},

    {47301u, "沖縄県国頭村"},

    {47302u, "沖縄県大宜味村"},

    {47303u, "沖縄県東村"},

    {47306u, "沖縄県今帰仁村"},

    {47308u, "沖縄県本部町"},

    {47311u, "沖縄県恩納村"},

    {47313u, "沖縄県宜野座村"},

    {47314u, "沖縄県金武町"},

    {47315u, "沖縄県伊江村"},

    {47324u, "沖縄県読谷村"},

    {47325u, "沖縄県嘉手納町"},

    {47326u, "沖縄県北谷町"},

    {47327u, "沖縄県北中城村"},

    {47328u, "沖縄県中城村"},

    {47329u, "沖縄県西原町"},

    {47348u, "沖縄県与那原町"},

    {47350u, "沖縄県南風原町"},

    {47353u, "沖縄県渡嘉敷村"},

    {47354u, "沖縄県座間味村"},

    {47355u, "沖縄県粟国村"},

    {47356u, "沖縄県渡名喜村"},

    {47357u, "沖縄県南大東村"},

    {47358u, "沖縄県北大東村"},

    {47359u, "沖縄県伊平屋村"},

    {47360u, "沖縄県伊是名村"},

    {47361u, "沖縄県久米島町"},

    {47362u, "沖縄県八重瀬町"},

    {47375u, "沖縄県多良間村"},

    {47381u, "沖縄県竹富町"},

    {47382u, "沖縄県与那国町"},
};
inline const char* qzss_dcx_ex1_target_area_code_ja_lookup(uint16_t id) {
    int lo = 0, hi = 1980 - 1;
    while (lo <= hi) {
        int mid = (lo + hi) / 2;
        if (QZSS_DCX_EX1_TARGET_AREA_CODE_JA_TABLE[mid].id == id) return QZSS_DCX_EX1_TARGET_AREA_CODE_JA_TABLE[mid].label;
        if (QZSS_DCX_EX1_TARGET_AREA_CODE_JA_TABLE[mid].id  < id) lo = mid + 1;
        else hi = mid - 1;
    }
    return nullptr;
}

} // namespace def
} // namespace azaraC
