...
struct NankaiPageBuffer {
    ...
    bool addPage(uint8_t page_num, uint8_t total_pages, const uint8_t* text_data) {
...
public:
    ...
    NankaiPageBuffer* addPage(const NankaiPageKey& key, uint8_t page_num, 
        ...
        // Add page to buffer
        if (_buffers[idx].addPage(page_num, total_pages, text_data)) {
            // Check if complete
