...
def test_scenario3():
    ...
    assert type(report) is reports.dcr.NankaiTroughEarthquake
...
def test_scenario6():
    ...
    assert type(report) is reports.dcr.Weather
    ...
    assert type(report) is reports.dcr.Flood
...
def test_northwest_pacific_tsunami():
    ...
    assert type(report) is reports.dcr.NorthwestPacificTsunami
...
def test_from_file():
    ...
    with open(dir_path + '/qzqsm_220307.log', encoding='utf-8') as f:
        while True:
            try:
